...
WebKit2 4.1 typelib is missing. On Ubuntu 24.04+ it's `gir1.2-webkit2-4.1`. Older distros ship `4.0` — VertexWrite does not support that version.
...
1. Is GPU acceleration broken? Try: `WEBKIT_DISABLE_COMPOSITING_MODE=1 vertexwrite`.
2. Check stderr for WebKit crash messages.
3. Disable custom CSS: `mv ~/.config/vertexwrite/custom.css{,.disabled}` and relaunch.
...
- If `python3-html2text` is not installed, VertexWrite uses a built-in fallback that handles common tags only (`h1-h6`, `p`, `br`, `strong/b`, `em/i`, `code`, `pre`, `a`, `img`, `ul/ol/li`, `blockquote`, `hr`). Anything else comes through as plain text. Install `python3-html2text` for best results.
...
1. Path must be exactly `~/.config/vertexwrite/custom.css` (or `$XDG_CONFIG_HOME/vertexwrite/custom.css`).
...
- `_write_to` writes a snapshot on every successful save. Check `~/.local/state/vertexwrite/snapshots/` exists and is writable.
...
- PDF engine: VertexWrite passes `--pdf-engine=xelatex`. Install a LaTeX stack: `sudo apt install texlive-xetex`.
